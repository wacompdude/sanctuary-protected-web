...
export async function getGroupDetail(
  ...
  const group = await getNotificationGroup(organizationId, groupId);
  ...
  const [
    ...
    listNotificationGroupMembers(organizationId, groupId),
    listDirectChildGroups(organizationId, groupId),
    listDirectParentGroups(organizationId, groupId),
    listNotificationGroupDefaults(organizationId, groupId),
    getNotificationGroupCounts(organizationId, group, { client: supabase }),
    includeEffective
      ? getEffectiveGroupUsers(organizationId, groupId, { client: supabase })
      : Promise.resolve([] as EffectiveGroupUser[]),
    listNotificationGroups(organizationId, { includeArchived: false }),
...
export async function isUserEffectiveGroupMember(params: {
  ...
  const users = await getEffectiveGroupUsers(params.organizationId, params.groupId);
